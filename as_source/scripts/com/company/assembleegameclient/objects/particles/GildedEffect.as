package com.company.assembleegameclient.objects.particles
{
   ...
   public class GildedEffect extends ParticleEffect
   {
      ...
      private function addParticles() : void
      {
         ...
         var _loc1_:int = 0;
         while(_loc1_ < this.numArm)
         {
            _loc2_ = _loc1_ / this.numArm - this.particlesOffset;
            _loc3_ = Math.cos(_loc2_) * this.rad_;
            _loc4_ = Math.sin(_loc2_) * this.rad_;
            _loc5_ = new GildedParticle(this.go_,_loc3_,_loc4_,_loc2_,this.rad_,this.healEffectDelay,this.color1_,this.color2_,this.color3_);
            map_.addObj(_loc5_,x_ + _loc3_,y_ + _loc4_);
            _loc1_++;
         }
      }
   }
