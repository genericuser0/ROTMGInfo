package com.greensock.core
{
   ...
   public class Animation
   {
      ...
      public function Animation(param1:Number = 0, param2:Object = null)
      {
         ...
         _duration = _totalDuration = Number(param1) || Number(0);
         _delay = Number(Number(this.vars.delay)) || Number(0);
         _timeScale = 1;
         _totalTime = _time = 0;
         data = this.vars.data;
         _rawPrevTime = -1;
      ...
      public function timeScale(param1:Number = NaN) : *
      {
         ...
         {
            _loc3_ = _pauseTime || _pauseTime == 0?Number(_pauseTime):Number(_timeline._totalTime);
      ...
      public function restart(param1:Boolean = false, param2:Boolean = true) : *
      {
         reversed(false);
         paused(false);
         return totalTime(!!param1?Number(-_delay):Number(0),param2,true);
      }
      ...
      public function paused(param1:Boolean = false) : *
      {
         ...
         if(param1 != _paused)
         {
            if(_timeline)
            {
               _loc3_ = _timeline.rawTime();
               _loc4_ = _loc3_ - _pauseTime;
               if(!param1 && _timeline.smoothChildTiming)
               {
                  _startTime = _startTime + _loc4_;
                  _uncache(false);
               }
               _pauseTime = !!param1?Number(_loc3_):Number(NaN);
               _paused = param1;
               _active = !param1 && _totalTime > 0 && _totalTime < _totalDuration;
               if(!param1 && _loc4_ != 0 && _initted && duration() !== 0)
               {
                  render(!!_timeline.smoothChildTiming?Number(_totalTime):Number((_loc3_ - _startTime) / _timeScale),true,true);
               }
            }
         }
         if(_gc && !param1)
      ...
      public function reversed(param1:Boolean = false) : *
      {
         ...
         {
            _reversed = param1;
            totalTime(_timeline && !_timeline.smoothChildTiming?Number(totalDuration() - _totalTime):Number(_totalTime),true);
         }
         return this;
      }
