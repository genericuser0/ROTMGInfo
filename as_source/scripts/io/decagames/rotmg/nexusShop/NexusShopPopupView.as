package io.decagames.rotmg.nexusShop
{
   ...
   public class NexusShopPopupView extends ModalPopup
   {
      ...
      private function addItemContainer() : void
      {
         ...
         {
            _loc2_ = new ItemWithTooltip(this.owner_.getSellableType(),80);
         }
         _loc2_.x = WIDTH / 2 - _loc2_.width / 2;
         _loc2_.y = HEIGHT / 2 - _loc2_.height + 5;
         addChild(_loc2_);
         var _loc1_:String = this.owner_.soldObjectName();
         this.nameText_ = new TextFieldDisplayConcrete().setSize(14).setColor(16777215);
         this.nameText_.setBold(true);
         this.nameText_.setStringBuilder(new LineBuilder().setParams(_loc1_));
         this.nameText_.setAutoSize(TextFieldAutoSize.CENTER);
         addChild(this.nameText_);
         this.nameText_.x = WIDTH / 2 - this.nameText_.width / 2;
         this.nameText_.y = _loc2_.y + 55;
      }
      ...
      private function addBuyButton() : void
      {
         ...
         {
            _loc2_ = 1;
            while(_loc2_ <= this.availableInventoryNumber)
            {
               _loc1_.push(_loc2_);
               _loc2_++;
            }
         ...
         this.spinner = new FixedNumbersSpinner(TextureParser.instance.getSliceScalingBitmap("UI","spinner_up_arrow"),0,_loc1_,"x");
