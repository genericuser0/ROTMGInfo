package io.decagames.rotmg.pets.data
{
   ...
   public class PetsModel
   {
      ...
      {
         return !!Boolean(this.seasonalEventModel.isChallenger)?0:int(int(this.yardXmlData.Fame));
      }
      
      public function getPetYardUpgradeGoldPrice() : int
      {
         return !!Boolean(this.seasonalEventModel.isChallenger)?0:int(int(this.yardXmlData.Price));
      }
      ...
      public function parsePetsData() : void
      {
         ...
         var _loc2_:int = 0;
         var _loc3_:XML = null;
         var _loc4_:SkinVO = null;
         if(this.petsData == null)
         {
            this.petsData = XML(new petsDataXML()).Object;
            _loc1_ = this.petsData.length();
            _loc2_ = 0;
            while(_loc2_ < _loc1_)
            {
               _loc3_ = this.petsData[_loc2_];
               if(_loc3_.hasOwnProperty("PetSkin"))
               {
                  if(_loc3_.@type != "0x8090")
                  {
                     _loc4_ = SkinVO.parseFromXML(_loc3_);
                     _loc4_.isOwned = this.ownedSkinsIDs.indexOf(_loc4_.skinType) >= 0;
                     this.skins[_loc4_.skinType] = _loc4_;
                     this._totalPetsSkins++;
                     if(!this.familySkins[_loc4_.family])
                     {
                        this.familySkins[_loc4_.family] = new Vector.<SkinVO>();
                     }
                     this.familySkins[_loc4_.family].push(_loc4_);
                  }
               }
               _loc2_++;
            }
