package io.decagames.rotmg.pets.windows.yard.list
{
   ...
   public class PetYardListMediator extends Mediator
   {
      ...
      override public function initialize() : void
      {
         ...
         var _loc1_:PetRarityEnum = PetRarityEnum.selectByOrdinal(this.model.getPetYardType() - 1);
         var _loc2_:PetRarityEnum = PetRarityEnum.selectByOrdinal(this.model.getPetYardType());
         this.view.showPetYardRarity(_loc1_.rarityName,_loc1_.ordinal < PetRarityEnum.DIVINE.ordinal && this.account.isRegistered());
         if(this.view.upgradeButton)
         {
            this.view.upgradeButton.clickSignal.add(this.upgradeYard);
            this.toolTip = new TextToolTip(3552822,10197915,"Upgrade Pet Yard","Upgrading your yard allows you to fuse pets up to " + _loc2_.rarityName,180);
      ...
      private function refreshPetsList() : void
      {
         ...
         this.petsList = new Vector.<PetItem>();
         var _loc1_:Vector.<PetVO> = this.model.getAllPets();
         _loc1_ = _loc1_.sort(this.sortByFamily);
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = this.petIconFactory.create(_loc2_,40,5526612,1);
            _loc3_.addEventListener(MouseEvent.CLICK,this.onPetSelected);
