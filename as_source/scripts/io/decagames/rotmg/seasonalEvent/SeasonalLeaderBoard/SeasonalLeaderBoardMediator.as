package io.decagames.rotmg.seasonalEvent.SeasonalLeaderBoard
{
   ...
   public class SeasonalLeaderBoardMediator extends Mediator
   {
      ...
      public static const REFRESH_INTERVAL_IN_MILLISECONDS:Number = 5 * 60 * 1000;
      ...
      {
         var _loc1_:Date = this.view.tabs.currentTabLabel == SeasonalLeaderBoard.TOP_20_TAB_LABEL?this.seasonalEventModel.leaderboardTop20CreateTime:this.seasonalEventModel.leaderboardPlayerCreateTime;
      ...
      private function updateRefreshTime() : void
      {
         ...
         this.view.refreshTime.text = REFRESH_TIME + _loc1_;
      }
      
      private function getTimeFormat(param1:Number) : String
      {
         var _loc2_:Date = new Date(param1);
         var _loc3_:String = _loc2_.getUTCHours() > 9?_loc2_.getUTCHours().toString():"0" + _loc2_.getUTCHours();
         var _loc4_:String = _loc2_.getUTCMinutes() > 9?_loc2_.getUTCMinutes().toString():"0" + _loc2_.getUTCMinutes();
         var _loc5_:String = _loc2_.getUTCSeconds() > 9?_loc2_.getUTCSeconds().toString():"0" + _loc2_.getUTCSeconds();
      ...
      private function onTabSelected(param1:String) : void
      {
         ...
         else if(param1 == SeasonalLeaderBoard.TOP_20_TAB_LABEL)
         {
            this.upDateTop20();
         }
         else if(param1 == SeasonalLeaderBoard.PLAYER_TAB_LABEL)
         {
      ...
      private function timeToRefresh() : Number
      {
         ...
         var _loc1_:Date = this.view.tabs.currentTabLabel == SeasonalLeaderBoard.TOP_20_TAB_LABEL?this.seasonalEventModel.leaderboardTop20RefreshTime:this.seasonalEventModel.leaderboardPlayerRefreshTime;
         if(_loc1_)
         {
            _loc2_ = new Date();
            return _loc1_.time - _loc2_.time;
         }
         return 0;
      }
      ...
      private function upDateTop20() : void
      {
         ...
         this.view.spinner.visible = false;
         this.view.spinner.pause();
         var _loc1_:Vector.<SeasonalLeaderBoardItemData> = this.seasonalEventModel.leaderboardTop20ItemDatas;
      ...
      private function upDatePlayerPosition() : void
      {
         ...
         this.view.spinner.visible = false;
         this.view.spinner.pause();
         var _loc1_:Vector.<SeasonalLeaderBoardItemData> = this.seasonalEventModel.leaderboardPlayerItemDatas;
         for each(_loc2_ in _loc1_)
         {
