package io.decagames.rotmg.shop
{
   ...
   public class ShopPopupMediator extends Mediator
   {
      ...
      private function updateMysteryBoxes() : void
      {
         ...
         var _loc3_:GenericBoxTile = null;
         this.mysteryBoxesGrid.clearGrid();
         var _loc1_:Vector.<MysteryBoxInfo> = this.mysteryBoxModel.getBoxesForGrid();
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null && (!_loc2_.endTime || _loc2_.getSecondsToEnd() > 0))
            {
               _loc3_ = this.createBoxTile(_loc2_,MysteryBoxTile);
      ...
      private function updatePackages() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null && (!_loc2_.endTime || _loc2_.getSecondsToEnd() > 0))
            {
               _loc3_ = this.createBoxTile(_loc2_,PackageBoxTile);
      ...
      override public function initialize() : void
      {
         ...
         for each(_loc4_ in _loc1_)
         {
            ...
            {
               if(_loc4_.isNew() && (_loc4_.startTime.getTime() > _loc2_.getTime() || !Parameters.data_["packages_indicator"]))
               {
                  _loc3_ = true;
               }
            }
