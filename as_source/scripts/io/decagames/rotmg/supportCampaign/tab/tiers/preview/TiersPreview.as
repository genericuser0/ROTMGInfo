      ...
      private function renderButtons(param1:int, param2:int, param3:int) : void
      {
         ...
         if(param1 > param3 && param1 != this._ranks.length + 1)
         {
            ...
            else
            {
               ...
               _loc4_.text = this._ranks[param1 - 1].toString();
               _loc4_.x = this.donateButtonBackground.x + Math.round((this.donateButtonBackground.width - _loc4_.width) / 2) - 10;
               _loc4_.y = this.donateButtonBackground.y + 13;
         ...
         else if(param3)
         {
            this.requiredPointsContainer = new Sprite();
            _loc5_ = new UILabel();
            DefaultLabelFormat.createLabelFormat(_loc5_,22,4958208,TextFormatAlign.CENTER,true);
            this.requiredPointsContainer.addChild(_loc5_);
            _loc5_.text = "Claimed";
            _loc5_.x = Math.round((this._componentWidth - _loc5_.width) / 2);
            _loc5_.y = 190;
            addChild(this.requiredPointsContainer);
         }
      }
