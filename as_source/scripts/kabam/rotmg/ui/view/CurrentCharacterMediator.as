   ...
   public class CurrentCharacterMediator extends Mediator
   {
      ...
      override public function initialize() : void
      {
         ...
         if(this.seasonalEventModel.scheduledSeasonalEvent)
         {
            ...
            {
               _loc1_ = Parameters.data_[PopupNamesConfig.CHALLENGER_INFO_POPUP];
               _loc2_ = new Date().time;
               if(_loc2_ - (_loc1_ + this.DAY_IN_MILLISECONDS) > 0)
               {
                  this.showSeasonsComingPopup();
                  Parameters.data_[PopupNamesConfig.CHALLENGER_INFO_POPUP] = _loc2_;
               }
            }
            else
            {
