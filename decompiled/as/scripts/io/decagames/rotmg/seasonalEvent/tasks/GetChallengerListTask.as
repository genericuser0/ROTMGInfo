package io.decagames.rotmg.seasonalEvent.tasks
{
   ...
   public class GetChallengerListTask extends BaseTask
   {
      ...
      override protected function startTask() : void
      {
         ...
         else if(this.listType == SeasonalLeaderBoard.PLAYER_TAB_LABEL)
         {
      ...
      private function updateFameListData(param1:String) : void
      {
         ...
         var _loc3_:Date = new Date(_loc2_.GeneratedOn * 1000);
         var _loc4_:Number = _loc3_.getTimezoneOffset() * 60 * 1000;
         _loc3_.setTime(_loc3_.getTime() - _loc4_);
         var _loc5_:Date = new Date();
         _loc5_.setTime(_loc5_.getTime() + REFRESH_INTERVAL_IN_MILLISECONDS);
         ...
         if(this.listType == SeasonalLeaderBoard.TOP_20_TAB_LABEL)
         {
            ...
            this.seasonalEventModel.leaderboardTop20RefreshTime = _loc5_;
            this.seasonalEventModel.leaderboardTop20CreateTime = _loc3_;
         }
         else if(this.listType == SeasonalLeaderBoard.PLAYER_TAB_LABEL)
         {
            _loc6_.sort(this.fameSort);
            this.seasonalEventModel.leaderboardPlayerItemDatas = _loc6_;
            this.seasonalEventModel.leaderboardPlayerRefreshTime = _loc5_;
            this.seasonalEventModel.leaderboardPlayerCreateTime = _loc3_;
         }
         completeTask(true);
      }
