package io.decagames.rotmg.ui
{
   ...
   public class PetFeedProgressBar extends UIGridElement
   {
      ...
      private function render(param1:int, param2:int) : void
      {
         ...
         var _loc4_:int = this._maxValue - AbilitiesUtil.abilityPowerToMinPoints(this._currentLevel);
         var _loc5_:int = param1 - AbilitiesUtil.abilityPowerToMinPoints(this._currentLevel);
         var _loc6_:Number = this._currentLevel == this._maxLevel?Number(1):Number(_loc5_ / _loc4_);
         if(this._animateLevelProgress)
         {
         ...
         {
            this.drawProgress(_loc6_);
         }
         if(_loc3_)
         {
            if(param2 >= this._maxValue)
            {
               _loc9_ = AbilitiesUtil.abilityPointsToLevel(param2);
               if(_loc9_ >= this._maxLevel)
               {
                  _loc9_ = this._maxLevel;
                  this.drawSimulatedProgress(this._componentWidth,this.MAX_COLOR);
                  this.setLevelLabel(_loc9_);
               }
               else
               {
                  _loc10_ = AbilitiesUtil.abilityPowerToMinPoints(_loc9_ + 1);
                  _loc11_ = AbilitiesUtil.abilityPowerToMinPoints(_loc9_);
                  _loc8_ = _loc10_ - _loc11_;
                  _loc7_ = param2 - _loc11_;
                  this.drawSimulatedProgress(_loc7_ / _loc8_);
                  this.setLevelLabel(_loc9_);
               }
            }
            else
            {
               _loc7_ = param2 - AbilitiesUtil.abilityPowerToMinPoints(this._currentLevel);
               _loc12_ = _loc7_ / _loc4_;
               this.drawSimulatedProgress(_loc12_,_loc6_);
            }
         }
