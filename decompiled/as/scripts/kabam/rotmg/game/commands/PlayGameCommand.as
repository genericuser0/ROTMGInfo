package kabam.rotmg.game.commands
{
   ...
   public class PlayGameCommand
   {
      ...
      private function makeGameView() : void
      {
         ...
         var _loc2_:SavedCharacter = this.model.getCharacterById(this.data.charId);
         if(_loc2_)
         {
            _loc1_ = Boolean(int(_loc2_.charXML_.IsChallenger));
         }
         else
         {
            _loc1_ = Boolean(this.seasonalEventModel.isChallenger);
         }
         var _loc3_:int = !!_loc1_?int(Server.CHALLENGER_SERVER):int(Server.NORMAL_SERVER);
         this.serverModel.setAvailableServers(_loc3_);
         var _loc4_:Server = this.data.server || this.serverModel.getServer();
         var _loc5_:int = !!this.data.isNewGame?int(this.getInitialGameId()):int(this.data.gameId);
         var _loc6_:Boolean = this.data.createCharacter;
         var _loc7_:int = this.data.charId;
         var _loc8_:int = !!this.data.isNewGame?-1:int(this.data.keyTime);
         var _loc9_:ByteArray = this.data.key;
         this.model.currentCharId = _loc7_;
         this.setScreen.dispatch(new GameSprite(_loc4_,_loc5_,_loc6_,_loc7_,_loc8_,_loc9_,this.model,null,this.data.isFromArena));
      }
