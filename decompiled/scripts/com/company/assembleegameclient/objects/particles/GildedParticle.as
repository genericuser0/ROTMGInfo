package com.company.assembleegameclient.objects.particles
{
   ...
   public class GildedParticle extends Particle
   {
      ...
      override public function update(param1:int, param2:int) : Boolean
      {
         ...
         size_ = this.fSize;
         var _loc4_:Number = Quad.easeOut(_loc3_);
         var _loc5_:Number = 2 * Math.PI * (_loc4_ + this.armOffset);
         var _loc6_:Number = this.radius * (1 - _loc4_);
         var _loc7_:Number = _loc6_ * Math.cos(_loc5_);
         var _loc8_:Number = _loc6_ * Math.sin(_loc5_);
         moveTo(this.go.x_ + _loc7_,this.go.y_ + _loc8_);
         if(_loc3_ < 0.33)
         {
