package com.company.assembleegameclient.objects.particles
{
   ...
   public class OrbEffect extends ParticleEffect
   {
      ...
      private static function parseBitmapDataFromImageSet(param1:String) : Vector.<BitmapData>
      {
         ...
         var _loc5_:uint = _loc3_.images_.length;
         _loc4_ = 0;
         while(_loc4_ < _loc5_)
         {
            _loc6_ = TextureRedrawer.redraw(_loc3_.images_[_loc4_],120,true,11673446,true,5,11673446,1.4);
            if(_loc4_ == 8)
            {
               _loc6_ = apply(_loc6_,11673446);
            }
            else
            {
               _loc6_ = apply(_loc6_,3675232);
            }
            _loc2_.push(_loc6_);
            _loc4_++;
         }
