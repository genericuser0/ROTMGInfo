package io.decagames.rotmg.dailyQuests.view.slot
{
   ...
   public class DailyQuestItemSlot extends Sprite
   {
      ...
      private function renderItem() : void
      {
         ...
         var _loc1_:BitmapData = ObjectLibrary.getRedrawnTextureFromType(this._itemID,SLOT_SIZE * 2,true);
         _loc1_ = _loc1_.clone();
         var _loc2_:XML = ObjectLibrary.xmlLibrary_[this._itemID];
         this.bitmapFactory = StaticInjectorContext.getInjector().getInstance(BitmapTextFactory);
         if(_loc2_ && _loc2_.hasOwnProperty("Quantity") && this.bitmapFactory)
         {
            _loc3_ = this.bitmapFactory.make(new StaticStringBuilder(String(_loc2_.Quantity)),12,16777215,false,new Matrix(),true);
            _loc4_ = new Matrix();
