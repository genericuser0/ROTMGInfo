package io.decagames.rotmg.pets.windows.yard.list
{
   ...
   public class PetYardList extends Sprite
   {
      ...
      public function showPetYardRarity(param1:String, param2:Boolean) : void
      {
         ...
         _loc3_ = TextureParser.instance.getSliceScalingBitmap("UI","content_divider_smalltitle_white",180);
         Tint.add(_loc3_,3355443,1);
         addChild(_loc3_);
         _loc3_.x = Math.round((YARD_WIDTH - _loc3_.width) / 2);
         _loc3_.y = 23;
         _loc4_ = new UILabel();
         DefaultLabelFormat.petYardRarity(_loc4_);
         _loc4_.text = param1;
         ...
         if(param2)
         {
            ...
            this._upgradeButton.x = _loc3_.x + _loc3_.width - this._upgradeButton.width + 8;
            this._upgradeButton.y = _loc3_.y - this._upgradeButton.height / 2 + 8;
      ...
      private function createScrollview() : void
      {
         ...
         _loc1_.addChild(this.yardContainer);
         var _loc2_:UIScrollbar = new UIScrollbar(YARD_HEIGHT - 60);
         _loc2_.mouseRollSpeedFactor = 1;
         _loc2_.scrollObject = this;
         _loc2_.content = this.yardContainer;
