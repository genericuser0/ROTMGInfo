package io.decagames.rotmg.shop.packages
{
   ...
   public class PackageBoxTileMediator extends Mediator
   {
      ...
      private function onRollRequestComplete(param1:Boolean, param2:*) : void
      {
         ...
         {
            _loc5_ = "MysteryBoxRollModal.pleaseTryAgainString";
            ...
            if(param2.indexOf("MysteryBoxError.soldOut") >= 0)
            {
               ...
               {
                  _loc7_ = _loc6_[1];
                  this.view.boxInfo.unitsLeft = _loc7_;
                  if(_loc7_ == 0)
                  {
                     _loc5_ = "MysteryBoxError.soldOutAll";
                  }
                  else
                  {
                     _loc5_ = LineBuilder.getLocalizedStringFromKey("MysteryBoxError.soldOutLeft",{
            ...
            if(param2.indexOf("MysteryBoxError.maxPurchase") >= 0)
            {
               ...
               if(_loc8_.length == 2)
               {
                  ...
                  if(_loc9_ == 0)
                  {
                     _loc5_ = "MysteryBoxError.maxPurchase";
                  }
                  else
                  {
                     _loc5_ = LineBuilder.getLocalizedStringFromKey("MysteryBoxError.maxPurchaseLeft",{"left":_loc9_});
                  }
               }
            }
            if(param2.indexOf("blockedForUser") >= 0)
            {
               ...
               if(_loc10_.length == 2)
               {
                  _loc5_ = LineBuilder.getLocalizedStringFromKey("MysteryBoxError.blockedForUser",{"date":_loc10_[1]});
               }
            }
            this.showErrorMessage(_loc5_);
         }
      }
